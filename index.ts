...
class Line {
    ...
    toString() { return `${this.a}x + ${this.b}y = ${this.c}${this.b == 0 ? '' : ` (y = ${-this.a/this.b}x + ${this.c/this.b})`}` }
}
...
const lineFromTwoPoint = ({x: x1, y: y1}: Point, { x: x2, y: y2}: Point) => new Line(y2-y1, x1-x2, x1*y2-x2*y1);
const intersectionOfTwoLine = ({a: a1, b: b1, c: c1}: Line, {a: a2, b: b2, c: c2}: Line) => new Point((c1*b2-c2*b1)/(a1*b2-a2*b1), (a1*c2-a2*c1)/(a1*b2-a2*b1));
...
const functions = {
    ...
    i: (a1, b1, c1, a2, b2, c2) => intersectionOfTwoLine(new Line(a1, b1, c1), new Line(a2, b2, c2)).toString(),
